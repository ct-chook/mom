...
class TestCase:
    ...
    def before(self):
        ...
        mapoptions.set_number_of_players(3)
        mapoptions.mapname = 'test'
        self.model = BoardModel(mapoptions)
        assert len(self.model.players) == 3
    ...
    def test_get_short_and_long_attacks(self, before):
        sphinx = Monster(
            MonsterType.SPHINX, (0, 0), self.player_1, Terrain.GRASS)
        hydra = Monster(
            MonsterType.HYDRA, (0, 0), self.player_2, Terrain.DUNE)
    ...
    def test_only_human_loses_lord(self, before):
    ...
    def test_one_out_of_three_players_loses(self, before):
        player_3 = self.model.players[2]
        green_lord = self.model.get_lord_of_player(player_3)
        blue_player = self.model.get_current_player()
        assert green_lord.type == Monster.Type.SORCERER
        self.model.on_end_turn()  # red is after
        self.model.on_end_turn()  # green is after
