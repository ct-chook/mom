...
class CombatWindow(Window):
    def __init__(self):
        ...
        self.event_queue: events.EventQueue = None
    ...
    def on_combat(self, attacks, attack_range):
        ...
        draw_screen_event = Event(self.view.draw_screen, attacks)
        show_attacks_event = Event(self.view.show_attack, self.combat_log)
        close_window_event = Event(self.handle_combat_end)
        clear_selection_event = Event(self.parent.view.clear_highlighted_tiles)
        combat_events = (
            draw_screen_event, show_attacks_event, close_window_event,
            clear_selection_event)
        self.event_queue = EventQueue(combat_events)

    def handle_mouseclick(self):
        self.event_queue.skip()
